...
class CrossNetV2(nn.Module):
    def __init__(self, input_dim,ouput_dim, num_layers,dtype=None):
        super(CrossNetV2, self).__init__()
        self.num_layers = num_layers
        self.cross_layers = nn.ModuleList(nn.Linear(input_dim, input_dim,dtype=dtype)
                                          for _ in range(self.num_layers))
        self.out = nn.Linear(input_dim,ouput_dim,dtype=dtype)
